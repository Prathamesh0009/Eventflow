...
EventFlow uses **TypeORM migrations** for schema versioning and evolution.
...
1. **Development:**
   ```bash
   npm run migration:generate -- -n AddEventStreamsTable
   ```

2. **Review:** Check generated migration file

3. **Test:** Run migrations on local database

4. **Deploy:** Migrations run automatically on deployment (CI/CD)
...
```typescript
// migrations/1234567890-CreateEventStreams.ts
export class CreateEventStreams1234567890 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    // Down migrations not used (forward-only policy)
  }
}
```
...
* **Staging/Production:** Automatic via CI/CD pipeline
